#!/usr/bin/env bash
# PCB-DSLR target domain (v3) — yolov10 progressive (progressive 0.25->0.35->0.45), Gen0->Gen3
# Teacher: runs/yolov10/baseline_noisy_v3/weights/best.pt (warm_start in config)
...
MODEL="yolov10"
CONFIG="configs/yolov10/self_train_dslr_progressive.yaml"
OUTPUT="experiments/yolov10_st_dslr_progressive_v3_42"
LOG="experiments/run_yolov10_dslr_progressive_v3.log"
...
echo "=== yolov10 PCB-DSLR progressive self-training (v3) ===" | tee "$LOG"
...
echo "--- self-training loop (progressive 0.25->0.35->0.45) ---" | tee -a "$LOG"
