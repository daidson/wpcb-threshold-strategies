#!/usr/bin/env bash
# PCB-DSLR target domain (v3) — yolo12 PATH A progressive (progressive 0.25->0.35->0.45), Gen0->Gen3
...
# Prerequisite: confirm the adaptive iter1 probe gate passed first.
set -euo pipefail

MODEL="yolo12"
CONFIG="configs/yolo12/self_train_dslr_pathA_progressive.yaml"
OUTPUT="experiments/yolo12_st_dslr_pathA_progressive_v3_42"
LOG="experiments/run_yolo12_dslr_pathA_progressive_v3.log"
LABELED_IMAGES="data/FICS_PCB_REMAP_NOISE_TRAIN/images"
LABELED_LABELS="data/FICS_PCB_REMAP_NOISE_TRAIN/labels"

echo "=== yolo12 PCB-DSLR PATH A progressive self-training (v3) ===" | tee "$LOG"
...
echo "--- self-training loop (progressive 0.25->0.35->0.45, support base retained) ---" | tee -a "$LOG"
