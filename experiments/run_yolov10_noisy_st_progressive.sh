#!/usr/bin/env bash
# Noise experiment — YOLOv10 progressive threshold (0.25 → 0.35 → 0.45 → 0.55, seed=42)
# Training set: data/FICS_PCB_REMAP_NOISE/ (75% annotations corrupted — 25% class-swap + 25% removal + 25% box-distortion)
# Prerequisite: configs/yolov10/base_noisy.yaml must have been trained first
set -euo pipefail

MODEL="yolov10"
CONFIG="configs/yolov10/self_train_noisy_progressive.yaml"
OUTPUT="experiments/yolov10_st_noisy_progressive_42"
LOG="experiments/run_yolov10_noisy_st_progressive.log"
...
echo "=== YOLOv10 noise experiment — progressive self-training ===" | tee "$LOG"
