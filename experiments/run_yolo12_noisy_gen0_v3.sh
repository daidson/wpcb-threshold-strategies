...
# Checkpoint lands at: runs/yolo12/baseline_noisy_v3/weights/best.pt
# Prerequisite: yolo12m.pt must be present in repo root.
set -euo pipefail

MODEL="yolo12"
CONFIG="configs/yolo12/base_noisy.yaml"
LOG="experiments/run_yolo12_noisy_gen0_v3.log"

echo "=== YOLO12m noisy GEN0 baseline training (v3) ===" | tee "$LOG"
...
echo "Checkpoint expected at: runs/yolo12/baseline_noisy_v3/weights/best.pt" | tee -a "$LOG"
