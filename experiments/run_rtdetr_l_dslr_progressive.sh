#!/usr/bin/env bash
# PCB-DSLR target domain — RT-DETR-l progressive threshold (0.25→0.35→0.45→0.55, seed=42)
# GEN0 teacher: runs/rtdetr_l/baseline_noisy/weights/best.pt (FICS-noisy trained)
# IC GT merged via partial_labels_dir; pseudo-labels for all other classes from teacher
set -euo pipefail

MODEL="rtdetr_l"
CONFIG="configs/rtdetr_l/self_train_dslr_progressive.yaml"
OUTPUT="experiments/rtdetr_l_st_dslr_progressive_42"
LOG="experiments/run_rtdetr_l_dslr_progressive.log"
...
echo "=== RT-DETR-l PCB-DSLR — progressive self-training ===" | tee "$LOG"
